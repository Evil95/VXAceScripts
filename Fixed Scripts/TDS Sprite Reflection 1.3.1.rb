...
#------------------------------------------------------------------------------
#   Fix by Evil95
#   - If you want to remove the reflection from Follower 1, nothing happend.
#     -> This is now fixed! (Line 185-207)
#------------------------------------------------------------------------------
#  * Description:
  ...
  def change_reflection_state(id, state)
    # If ID is less than 1
      if id == 0
        ...
        $game_player.followers[0].reflection_active = state       
      elsif id == -2
        # Set Reflection State for Followers
        $game_player.followers[1].reflection_active = state        
      elsif id == -3
        # Set Reflection State for Followers
        $game_player.followers[2].reflection_active = state
      elsif id > 0
...
class Sprite_Character_Reflection < Sprite_Character
  ...
  def initialize(viewport, character = nil)
    ...
    self.mirror = true ; self.angle = 180 ; self.opacity = 80#160    
